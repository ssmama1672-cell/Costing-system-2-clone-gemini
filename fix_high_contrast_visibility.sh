...
echo "==> 1. Updating RMPriceMatrixPage.jsx with high-contrast text, clear dropdown options, and bold tables..."
    ...
    <div className="space-y-4 text-xs font-sans">
      ...
      <div className="bg-white p-3 rounded-2xl border border-slate-200 shadow-xs flex flex-wrap justify-between items-center gap-3">
        <div className="flex items-center gap-2">
          ...
          >
            {vendors.map(v => (
              <option key={v.vendorId} value={v.vendorId} className="text-slate-900">{v.vendorName}</option>
            ))}
          </select>
        </div>

        <div className="flex items-center gap-2">
          <span className="text-slate-800 font-bold">Period: From</span>
          <input 
            type="date" 
            value={periodFrom} 
                      ...
                      <tr key={m.id} className="hover:bg-slate-50 transition">
                          ...
                          <div className="inline-flex items-center bg-amber-50 border border-amber-300 rounded-lg px-2 py-1 shadow-2xs">
                            ...
                            <input
                              ...
                              className={`w-16 bg-transparent font-black text-amber-950 text-center outline-hidden ${
                                isRowDisabled ? 'cursor-not-allowed text-amber-950' : 'cursor-text'
                              }`}
                            />
                          </div>
                        </td>
                        ...
                        <td className="py-3 px-4">
                          <div className="space-y-1.5">
                            ...
                            >
                              <option value={m.approvedCode} className="text-slate-900 font-bold">{m.approvedCode} (Contract Baseline)</option>
                              {allPurchasedGradeOptions.map((opt, i) => (
                                <option key={i} value={opt.code} className="text-slate-900 font-semibold">{opt.label}</option>
                              ))}
                            </select>
                            <div className="flex items-center gap-2">
                        ...
                        <td className="py-3 px-4">
                          <div className="space-y-1.5">
                            <select
                              ...
                              className="w-full px-2 py-1 rounded-lg border border-slate-300 font-bold text-xs bg-white text-slate-900 shadow-2xs"
                            >
                              <option value="" className="text-slate-700">Select Alternate Lot 2</option>
                              <option value={m.approvedCode} className="text-slate-900 font-bold">{m.approvedCode} (Contract Baseline)</option>
                              {allPurchasedGradeOptions.map((opt, i) => (
                                <option key={i} value={opt.code} className="text-slate-900 font-semibold">{opt.label}</option>
                              ))}
                        ...
                        <td className="py-3 px-4">
                          <div className="space-y-1.5">
                            <select
                              ...
                              className="w-full px-2 py-1 rounded-lg border border-slate-300 font-bold text-xs bg-white text-slate-900 shadow-2xs"
                            >
                              <option value="" className="text-slate-700">Select Alternate Lot 3</option>
                              <option value={m.approvedCode} className="text-slate-900 font-bold">{m.approvedCode} (Contract Baseline)</option>
                              {allPurchasedGradeOptions.map((opt, i) => (
                                <option key={i} value={opt.code} className="text-slate-900 font-semibold">{opt.label}</option>
                              ))}
      ...
      {/* TAB 2: DAY-WISE PURCHASES (HIGH CONTRAST BOLD STYLING) */}
      {activeTab === 'purchases' && (
      ...
      {/* TAB 3: DAY-WISE SALES (HIGH CONTRAST BOLD STYLING) */}
      {activeTab === 'sales' && (
        <div className="space-y-4">
          <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-xs space-y-3">
            <div className="flex flex-wrap justify-between items-center gap-2 border-b border-slate-100 pb-3">
            ...
            <form onSubmit={handleAddSales} className="flex flex-wrap items-center gap-2">
              ...
              >
                {vendors.map(v => (
                  <option key={v.vendorId} value={v.vendorId} className="text-slate-900 font-semibold">{v.vendorName}</option>
                ))}
              </select>
              <input 
                type="text" 
...
echo "==> 2. Updating MISVariancePage.jsx with crisp, high-contrast dark bold typography..."
...
export default function MISVariancePage() {
  ...
  // Calculate Product Realization Summary
              ...
              {productSummaryList.length === 0 ? (
                ...
                productSummaryList.map((p, idx) => (
                  <tr key={idx} className="hover:bg-slate-50 transition font-medium">
                    ...
                    <td className="py-2.5 px-2 text-center font-mono font-bold text-slate-800">{p.invoicesCount}</td>
          ...
          <table className="w-full text-left border-collapse text-xs">
            <thead className="bg-slate-100 text-slate-800 uppercase font-bold text-[10px] border-b border-slate-300">
              ...
              <tr className="border-t border-slate-300 text-[9px]">
                <th className="py-1.5 px-3 text-right text-slate-900 font-bold">Total Sales Revenue</th>
                <th className="py-1.5 px-3 text-right border-r border-slate-300 text-slate-900 font-bold">Cost Variance Gain / Loss</th>
                <th className="py-1.5 px-3 text-right text-slate-900 font-bold">Total Sales Revenue</th>
                <th className="py-1.5 px-3 text-right border-r border-slate-300 text-slate-900 font-bold">Cost Variance Gain / Loss</th>
                <th className="py-1.5 px-3 text-center text-slate-900 font-bold">Revenue Growth %</th>
                <th className="py-1.5 px-3 text-right text-slate-900 font-bold">Variance Delta (Δ)</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-200 font-medium">
              {vendorBreakdowns.map((vb, idx) => (
                <tr key={idx} className="hover:bg-slate-50 font-medium">
                  ...
                  <td className="py-2.5 px-3 text-right font-mono font-bold text-slate-700">₹0</td>
                  <td className="py-2.5 px-3 text-right font-mono font-bold text-slate-700 border-r border-slate-200">+ ₹0</td>
...
echo "-------------------------------------------------------------------"
echo "✅ HIGH CONTRAST & MATERIAL SANITIZATION SUCCESSFULLY APPLIED!"
echo "-------------------------------------------------------------------"
