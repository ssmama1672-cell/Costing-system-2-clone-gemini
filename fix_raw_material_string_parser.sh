...
echo "==> 1. Fixing exact Row 5 Raw Material Required string parser in BaselineMasterPage.jsx..."
...
export default function BaselineMasterPage() {
  ...
  const handleFileUpload = (e) => {
    ...
    reader.onload = (evt) => {
      ...
      // 1. Detect first data column index (skips S.N., Description, UOM)
      let dataStartCol = 3;
      ...
      for (let c = dataStartCol; c < totalCols; c++) {
        ...
        let shotWt = isHaierVendor ? 372 : 75.00;
        let reconWt = isHaierVendor ? 379.44 : 38.00;
        let tonnage = isHaierVendor ? 600 : 200;
        let tariff = isHaierVendor ? 4800 : 2000;
        let cycleTime = isHaierVendor ? 70 : 47;
        let haierOverheadPackage = 8.712912;
        let foamPolybag = 0;
        let plasticBin = 0;
        let freightCost = 0;
        let secondaryOp1 = 0;
        ...
        let assemblyCost = 0;
        let bopCost = 0;
        let mouldMaintenance = isHaierVendor ? 1.5 : 0.07;
        let qualityInspection = isHaierVendor ? 1.0 : 0.00;
        let iccReduce = isHaierVendor ? -0.29 : 0.00;
        let scrapAdj = 0;
        ...
        rawMatrix.forEach((r, rIdx) => {
          ...
          // 2. Exact Raw Material Required (Strictly ignores Cost/Rate rows)
          if (
          ...
          if (label.includes('shot weight') && !label.includes('reconciliation')) {
            const num = parseFloat(valStr);
            if (!isNaN(num) && num > 0) shotWt = num;
          }
          if (label.includes('reconciliation weight')) {
            const num = parseFloat(valStr);
            if (!isNaN(num) && num > 0) reconWt = num;
          }
          ...
          if (label.includes('foam / polybag')) foamPolybag = parseFloat(valStr) || 0;
          if (label.includes('plastic bin')) plasticBin = parseFloat(valStr) || 0;
          if (label.includes('freight cost') && !label.includes('mb freight')) freightCost = parseFloat(valStr) || 0;
          if (label.includes('secondary operation 1')) secondaryOp1 = parseFloat(valStr) || 0;
          if (label.includes('secondary operation 2')) secondaryOp2 = parseFloat(valStr) || 0;
          if (label.includes('screen printing - 1st')) screenPrint1 = parseFloat(valStr) || 0;
          if (label.includes('screen printing - 2nd')) screenPrint2 = parseFloat(valStr) || 0;
          if (label.includes('assembly cost')) assemblyCost = parseFloat(valStr) || 0;
          if (label.includes('insert / hinge')) bopCost = parseFloat(valStr) || 0;
          if (label.includes('mould maintenance')) mouldMaintenance = parseFloat(valStr) || mouldMaintenance;
          if (label.includes('quality inspection')) qualityInspection = parseFloat(valStr) || qualityInspection;
          if (label.includes('icc reduce')) iccReduce = parseFloat(valStr) || iccReduce;
          if (label.includes('scrap recovery adjustment')) scrapAdj = parseFloat(valStr) || 0;
          if (label.includes('packing cost')) packingCost = parseFloat(valStr) || packingCost;
          if (label.includes('transport cost')) transportCost = parseFloat(valStr) || transportCost;
        });
  ...
  if (activeStaged) {
    if (isHaierVendor) {
      ...
      haierStagedCalc = calculateHaierCost({
        ...
        cycleTime: ctApp,
        haierOverheadPackage: Number(activeStaged.haierOverheadPackage || 8.712912),
        ...
        mouldMaintenance: Number(activeStaged.mouldMaintenance || 1.5),
        qualityInspection: Number(activeStaged.qualityInspection || 1.0),
        iccReduce: Number(activeStaged.iccReduce || -0.29),
...
echo "-------------------------------------------------------------------"
echo "✅ RAW MATERIAL REQUIRED PARSER FIXED & LIVE ON DEV-V2!"
echo "-------------------------------------------------------------------"
