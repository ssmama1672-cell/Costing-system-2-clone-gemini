...
echo "==> 1. Updating BaselineMasterPage.jsx with accurate header offset detection and complete 38-line staging..."
...
export default function BaselineMasterPage() {
  ...
  const handleFileUpload = (e) => {
    ...
    reader.onload = (evt) => {
      ...
      // 1. DYNAMICALLY DETECT THE FIRST DATA COLUMN INDEX
      let dataStartCol = 3; // Default to Column D (Index 3) for Haier/Atomberg sheets
      ...
      for (let c = dataStartCol; c < totalCols; c++) {
        ...
        rawMatrix.forEach((r, rIdx) => {
          ...
          if (label.includes('raw material') || label.includes('rm grade')) {
            rmGradeStr = valStr;
          }
          ...
          if (label.includes('freight cost')) freightCost = parseFloat(valStr) || 0;
        ...
        // Fixed row fallback for standard templates
...
echo "-------------------------------------------------------------------"
echo "✅ HAIER STAGING S.N. & UOM OFFSET FIXED WITH FULL 38 LINES!"
echo "-------------------------------------------------------------------"
