...
echo "==> 1. Updating InlineEditModal.jsx with live editable inputs for Lines 25 through 37..."
...
export default function InlineEditModal({ product, onClose, onSave, onDelete }) {
  ...
  const [formData, setFormData] = useState({
    ...
    // Custom Melt Loss % & Machine Efficiency %
    ...
    // Haier Specific Baseline Lines (24 to 37)
    ...
    // Haier Specific Running Lines (24 to 37)
      ...
      <div className="bg-white rounded-2xl max-w-4xl w-full max-h-[92vh] flex flex-col shadow-2xl overflow-hidden text-xs">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2">
          {!isHaier ? (
            ...
            /* COMPLETE 38-LINE TABLE FOR HAIER (LINES 25 TO 37 ALL EDITABLE) */
            <table className="w-full text-left border-collapse text-xs">
              <thead className="bg-slate-100 text-slate-800 text-[10px] uppercase font-bold sticky top-0 border-b border-slate-300">
                <tr>
                  <th className="py-2.5 px-3 w-8">#</th>
              ...
              <tbody className="divide-y divide-slate-200 font-medium text-slate-900">
                ...
                {/* Rows 25 to 37: Fully Editable Inputs in Both Columns */}
                <tr>
                  <td className="py-1.5 px-3 font-mono text-slate-500">25</td>
...
echo "==> 2. Compiling build verification..."
npm run build
...
echo "-------------------------------------------------------------------"
echo "✅ LINES 25 THROUGH 37 ARE NOW LIVE EDITABLE INPUTS!"
echo "-------------------------------------------------------------------"
